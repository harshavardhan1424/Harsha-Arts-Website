...
const { verifyRazorpaySignature } = require('./razorpay-security');
...
const paymentLinksRouter = require('./payment-links-router');
        ...
        html: `
          <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 12px; color: #333;">
            ...
            <p style="margin-top: 30px; font-size: 0.9em; color: #888; text-align: center;">
              We will process your order soon. For any queries, contact us at +91 90001 94529.
            </p>
          </div>
        `
      });
...
app.post('/api/payment/razorpay/create-order', async (req, res) => {
  try {
    const { amount, items, address, phone, email, userId } = req.body;
...
app.post('/api/payment/razorpay/verify', async (req, res) => {
  try {
    ...
    // Step 6: Verify Payment Signature (Very Important)
    ...
    if (isSignatureValid) {
      ...
      const orders = JSON.parse(fs.readFileSync(ordersFile));
      const orderIndex = orders.findIndex(o => o.transactionId === razorpay_order_id);
...
app.post('/api/payment/initiate', (req, res) => {
  ...
  const payload = {
    ...
    redirectUrl: `${baseUrl}/payment-status.html?id=${transactionId}`,
